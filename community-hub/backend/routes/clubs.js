...
router.post('/', verifyToken, requireRole('systemAdmin', 'clubAdmin'), (req, res) => {
  const { clubName, description, category, contactEmail, membershipLink } = req.body;
  ...
  const query = `
    INSERT INTO clubs (clubName, description, category, adminID, contactEmail, membershipLink)
    VALUES (?, ?, ?, ?, ?, ?)
  `;

  db.query(query, [clubName, description, category, adminID, contactEmail, membershipLink], (err, result) => {
...
router.put('/:id', verifyToken, requireRole('clubAdmin', 'systemAdmin'), (req, res) => {
  ...
  db.query('SELECT * FROM clubs WHERE clubID = ?', [id], (err, results) => {
    ...
    const query = `
      UPDATE clubs
      SET clubName = ?, description = ?, category = ?, contactEmail = ?, membershipLink = ?
      WHERE clubID = ?
    `;
