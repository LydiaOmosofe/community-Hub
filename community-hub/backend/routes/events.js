...
router.post('/', verifyToken, requireRole('clubAdmin', 'systemAdmin'), (req, res) => {
  const { clubID, title, description, eventDate, venue } = req.body;

  if (!clubID || !title || !eventDate || !venue) {
    return res.status(400).json({ message: 'clubID, title, eventDate and venue are required.' });
  }

  const createdBy = req.user.userID;

  const query = `
    INSERT INTO events (clubID, title, description, eventDate, venue, createdBy)
    VALUES (?, ?, ?, ?, ?, ?)
  `;

  db.query(query, [clubID, title, description, eventDate, venue, createdBy], (err, result) => {
...
router.put('/:id', verifyToken, requireRole('clubAdmin', 'systemAdmin'), (req, res) => {
  ...
  db.query('SELECT * FROM events WHERE eventID = ?', [req.params.id], (err, results) => {
    ...
    const query = `
      UPDATE events
      SET title = ?, description = ?, eventDate = ?, venue = ?
      WHERE eventID = ?
    `;
