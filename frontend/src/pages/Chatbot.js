...
// Read user dynamically so it always reflects the current logged-in account
...
localStorage.setItem(key, JSON.stringify(updated));
  ...
  const newChat = () => { setMessages([]); setStarted(false); setInput(''); setActiveSession(null); };
  ...
  return (
    <div style={{ fontFamily: "'Segoe UI', system-ui, sans-serif", display: 'flex', height: 'calc(100vh - 66px)', overflow: 'hidden', background: '#f8fafc' }}>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        ...
        {/* Messages */}
        <div ref={msgsRef} style={{ flex: 1, overflowY: 'auto', padding: '28px 24px', display: 'flex', flexDirection: 'column', gap: 20 }}>
          {!started ? (
            /* Welcome screen */
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center', padding: 10 }}>
              ...
              <h2 style={{ fontSize: 22, fontWeight: 600, color: '#111', marginBottom: 8 }}>Hello there! 👋</h2>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, width: '100%', maxWidth: 500 }}>
                {SUGGESTIONS.map(s => (
                  <div key={s.q} onClick={() => send(s.q)} style={{ background: '#fff', border: '1px solid #e5e7eb', borderRadius: 12, padding: '14px 16px', cursor: 'pointer', textAlign: 'left', transition: 'all .15s' }}
                  ...
                  >
                    <p style={{ fontSize: 13, fontWeight: 500, color: '#111', marginBottom: 3 }}>{s.icon} {s.title}</p>
                    <p style={{ fontSize: 11.5, color: '#9ca3af' }}>{s.sub}</p>
                  </div>
                ))}
              </div>
            </div>
            ...
            messages.map((m, i) => (
              <div key={i} style={{ display: 'flex', gap: 10, alignItems: 'flex-end', flexDirection: m.role === 'user' ? 'row-reverse' : 'row' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: m.role === 'user' ? 'flex-end' : 'flex-start' }}>
                  <div style={{
                    maxWidth: 420, padding: '11px 16px', borderRadius: 16, fontSize: 13.5, lineHeight: 1.75,
        ...
        {/* Quick action chips */}
        <div style={{ display: 'flex', gap: 7, padding: '10px 24px', flexWrap: 'wrap', background: '#fff', borderTop: '1px solid #f3f4f6', flexShrink: 0 }}>
          {QUICK.map(q => (
            <span key={q.q} onClick={() => send(q.q)} style={{ background: '#f9fafb', border: '1px solid #e5e7eb', borderRadius: 20, padding: '6px 14px', fontSize: 12, color: '#6b7280', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 5, whiteSpace: 'nowrap', transition: '.15s' }}
